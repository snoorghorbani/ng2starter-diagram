...
import { DiagramConfigurationService } from "../diagram-module-configuration";
...
export class DiagramService {
  ...
  getDiagrams(): Observable<GetDiagramsApiModel.Response> {
    return this.http.get('http://localhost:3000/api/diagram')
  ...
  getGroups(): Observable<string[]> {
    return this.http.get('http://localhost:3000/api/diagram/groups')
      .map(response => (response as any).Result)
      .catch(err => {
        return Observable.throw(err)
      });
  ...
  getDiagram(id: string): Observable<any> {
    debugger;
    if (!id) debugger;
    return this.http.get(`http://localhost:3000/api/diagram/${id}`)
      .map(response => response)
      .catch(err => {
        return Observable.throw(err);
      });
  ...
  addDiagram(data: any): Observable<AddDiagramApiModel.Response> {
    ...
    debugger;
    return this.http.post('http://localhost:3000/api/diagram', model.getRequestBody())
      .map(response => response)
      .catch(err => {
        return Observable.throw(err)
      });
  ...
  updateDiagram(body: any): Observable<any> {
    debugger
    return this.http.put('http://localhost:3000/api/diagram', body)
      .map(response => response)
      .catch(err => {
        return Observable.throw(err)
      })
  ...
  deleteDiagram(id: string): Observable<any> {
    return this.http.delete(`http://localhost:3000/api/diagram/${id}`)
      .map(response => response)
      .catch(err => {
        return Observable.throw(err);
      })
    ...
    columnsMappings.forEach(item => {
      ...
      if (_.is.array(NameData)) {
        return res = res.concat(
          NameData.map((item, i) => [item].concat(ValueData[i]))
        );
      } else {
  ...
  charts = {};
  generateDiagram(config: any, id: string, route: string, sync: number) {
    this.charts[id] = c3.generate({
      ...config,
      bindto: `#diagram_${id}`
    });

    return this.getData({} as SourceModel, sync)
      .subscribe(data => {
        this.charts[id].load({
