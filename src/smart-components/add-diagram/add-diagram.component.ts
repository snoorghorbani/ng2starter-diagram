...
import { AddDiagramAction, ColumnAdded, DiagramConfigChangedAction, HaveEndpointAction, ColumnsMappingChangedAction, DataLoadedAction } from "../../actions";
...
export class AddDiagramComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.route.params.subscribe(params => {
      ...
      diagramId && this.diagramService.getDiagram(diagramId).subscribe(data => {
  ...
  ngOnInit() {
    this.formGroup.controls.ColumnMappings.statusChanges.subscribe(status => {
      if (status == 'VALID' && (this.formGroup.controls.ColumnMappings as FormArray).controls.length > 0 && this.data)
